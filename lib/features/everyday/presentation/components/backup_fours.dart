...
class BackupFours extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final Today currentlyUploadingToday =
        (backupProgressState as dynamic).currentlyUploadingToday;
    final unBackedupTodays = ref.watch(todayProvider.notifier).unBackedupTodays;
    final unbackedupTodaysWithoutCurrentlyUploadingToday = unBackedupTodays
        .where((element) => element != currentlyUploadingToday)
        .toList();
    final fours = [
      currentlyUploadingToday,
      ...unbackedupTodaysWithoutCurrentlyUploadingToday.take(3)
    ].reversed.toList();
                    ...
                    if (index == 3 && unBackedupTodays.length > 4)
                      GestureDetector(
                        onTap: () => context.navigator
                            .push(context.route(const UnbackedupScreen())),
